...
from rag_base.llm.langchain_llm import get_completion
from rag_base.llm.local_llm import LocalModels
from rag_base.database.create_db import create_db_info
from rag_base.qa_chain.Chat_QA_chain_self import Chat_QA_chain_self
from rag_base.qa_chain.QA_chain_self import QA_chain_self
import re
# 导入 dotenv 库的函数
...
LLM_MODEL_DICT = {
    ...
    "zhipuai": ["glm-4-plus", "glm-4-long", "glm-4-Air", "glm-4-flash"]
}
...
with block as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=6):
            ...
            msg = gr.Textbox(label="Prompt/输入")

            with gr.Row():
                # 创建提交按钮。
                db_with_his_btn = gr.Button("Chat with DB(RAG)")
